    ...
    <div class="box"><div class="box-body">
            <div class="btn-group" role="group">
                <?php echo TbHtml::button('<span class="fa fa-reply"></span> '.Yii::t('misc','Back'), array(
                    'submit'=>Yii::app()->createUrl('marketArea/index')));
                ?>
                <?php if ($model->scenario!='view'&&!in_array($model->status_type,array(8,10))): ?>
                    <?php echo TbHtml::button('<span class="fa fa-upload"></span> '.Yii::t('misc','Save'), array(
                        'submit'=>Yii::app()->createUrl('marketArea/save')));
                    ?>
                <?php endif ?>
                <?php if ($model->scenario=='edit'&&MarketFun::isAssignForArea($model)): ?>
            ...
            <div class="btn-group pull-right" role="group">
                <?php if ($model->scenario!='new'): ?>
                    <?php if (MarketFun::isAssignForArea($model)): ?>
        ...
        <div class="box-body">
            ...
            <?php if ($model->scenario!='new'): ?>
                <div class="form-group">
                    ...
                    <div class="col-sm-2">
                        <?php
                        echo TbHtml::textField("status_type",MarketFun::getStatusStrForArea($model),array("readonly"=>true));
                        ?>
                    </div>
                </div>
            <?php endif ?>
...
$js = "
$('table').on('change','[id^=\"MarketAreaForm\"]',function() {
	var n=$(this).attr('id').split('_');
	$('#MarketAreaForm_'+n[1]+'_'+n[2]+'_uflag').val('Y');
});
";
...
$('.btnAddRow').on('click',function() {
	...
	if (r>0) {
		...
		table.find('tr').eq(-1).find('[id*=\"MarketAreaForm_\"]').each(function(index) {
			var id = $(this).attr('id');
