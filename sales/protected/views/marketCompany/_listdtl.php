...
$idX = $this->record['id'];
?>
<tr class='clickable-row <?php echo $this->record['color']; ?>' data-href='<?php echo $this->getLink('MT01', 'marketCompany/edit', 'marketCompany/view', array('index'=>$this->record['id']));?>'>
    <td class="che">
        <?php if (MarketFun::isAssign($this->record)): ?>
            <input value="<?php echo $this->record['id']; ?>"  type="checkbox">
        <?php endif ?>
    </td>
    <td><?php echo $this->drawEditButton('MT01', 'marketCompany/edit', 'marketCompany/view', array('index'=>$this->record['id'])); ?></td>
...
if (count($this->record['detail'])>0) {
    foreach ($this->record['detail'] as $row) {
        ...
        $html.= "<td colspan='1' class='text-center'><strong>".Yii::t("market","info user")."：</strong>".$row["lcu"]."</td>";
