...
<section class="content">
	...
	<div class="btn-group" role="group">
        ...
        if(!empty($model->id)){
            $backUrl = Yii::app()->createUrl('contHead/detail',array("index"=>$model->id));
        ...
        echo TbHtml::button('<span class="fa fa-reply"></span> '.Yii::t('misc','Back'), array(
				'submit'=>$backUrl));
		?>
        <?php if ($model->scenario!='view'&&in_array($model->cont_status,array(0,9))): ?>
            <?php echo TbHtml::button('<span class="fa fa-save"></span> '.Yii::t('clue','draft'), array(
                'submit'=>Yii::app()->createUrl('contHead/save',array('type'=>'draft'))));
            ?>
            <?php echo TbHtml::button('<span class="fa fa-upload"></span> '.Yii::t('clue','need audit'), array(
                'submit'=>Yii::app()->createUrl('contHead/save',array('type'=>'audit'))));
            ?>
    ...
    <div class="row">
        <div class="col-lg-12">
            ...
            <?php $this->renderPartial('//cont/cont_form',array("model"=>$model,"form"=>$form)); ?>
        </div>
    </div>
</section>
