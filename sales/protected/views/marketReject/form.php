...
<section class="content-header">
    <h1>
        <strong><?php echo Yii::t('app','Market Reject'); ?></strong>
    </h1>
</section>
    ...
    <div class="box"><div class="box-body">
            <div class="btn-group" role="group">
                <?php echo TbHtml::button('<span class="fa fa-reply"></span> '.Yii::t('misc','Back'), array(
                    'submit'=>Yii::app()->createUrl('marketReject/index')));
                ?>
            </div>
            <div class="btn-group pull-right" role="group">
                <?php if ($model->scenario!='new'): ?>
                    <?php if ($model->scenario=='edit'&&$model->status_type==8): ?>
...
$this->renderPartial('//site/confirmDialog',array(
    'submit'=>Yii::app()->createUrl('marketReject/back'),
...
$js = "
$('table').on('change','[id^=\"MarketRejectForm\"]',function() {
	var n=$(this).attr('id').split('_');
	$('#MarketRejectForm_'+n[1]+'_'+n[2]+'_uflag').val('Y');
});
";
...
$('.btnAddRow').on('click',function() {
	...
	if (r>0) {
		...
		table.find('tr').eq(-1).find('[id*=\"MarketRejectForm_\"]').each(function(index) {
			var id = $(this).attr('id');
