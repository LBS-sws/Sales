...
class KABotController extends Controller 
{
	...
	public function accessRules()
	{
		return array(
			...
			array('allow', 
				'actions'=>array('index','view','downExcel','resetOldData','updateHistory','filedownload'),
...
//				$model->scenario = 'edit';
    ...
    public function actionFileupload($doctype) {
        ...
        if (isset($_POST['KABotForm'])) {
            ...
            $id = ($_POST['KABotForm']['scenario']=='new') ? 0 : $model->id;
    ...
    public function actionFileDownload($mastId, $docId, $fileId, $doctype) {
        $sql = "select id,city from sal_ka_bot where id = $docId";
        $row = Yii::app()->db->createCommand($sql)->queryRow();
        if ($row!==false) {
            $docman = new DocMan($doctype,$docId,'KABotForm');
