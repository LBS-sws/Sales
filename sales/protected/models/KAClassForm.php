...
class KAClassForm extends CFormModel
{
    /* User Fields */
    public $id;
    public $pro_name;
    ...
    public function retrieveData($index)
    {
        ...
        $sql = "select * from sal_ka_class where id=".$index." ";
        ...
        switch ($this->scenario) {
            ...
            case 'new':
                $sql = "insert into sal_ka_class(
						pro_name,level_id, z_index, z_display, lcu) values (
						:pro_name,0, :z_index, :z_display, :lcu)";
                break;
            case 'edit':
                $sql = "update sal_ka_class set 
					pro_name = :pro_name, 
					z_index = :z_index,
					z_display = :z_display,
					luu = :luu
    ...
    public function isOccupied($index) {
        if(is_numeric($index)){
            $sql = "select a.id from sal_ka_bot a where a.class_id=".$index." limit 1";
            $row = Yii::app()->db->createCommand($sql)->queryRow();
            $rtn = ($row !== false);
            return $rtn;
        }else{
    ...
    public static function getClassListForId($id=""){
        $list = array(""=>"");
        $rows = Yii::app()->db->createCommand()->select("pro_name,id")->from("sal_ka_class")
