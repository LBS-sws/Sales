...
class MarketSuccessForm extends MarketCompanyForm
{
    public function validateID($attribute, $params) {
        $this->addError($attribute, "已完成的资料无法修改，请退回至未分配！");
    }
    ...
    private function setMarketReady(){
        if(empty($this->ready_bool)){
            ...
            Yii::app()->db->createCommand()->update("sal_market", array(
                "luu"=>$uid,
                "ready_bool"=>1,
            ), "id=:id and status_type=10", array(":id" => $this->id));
    ...
    public function saveBack(){
        ...
        $update_html=array();
        $update_html[]="<span><b>完成退回至未分配</b></span>";
        $update_html = implode("<br/>",$update_html);
        $backHisSQL = array(
            "market_id" => $this->id,
            "lcu" => $uid,
	...
	public function isOccupied(){
        ...
        if(empty($this->id)||$this->status_type!=10){
