...
class KABusineForm extends CFormModel
{
	/* User Fields */
	public $id;
	public $pro_name;
	...
	public function retrieveData($index)
	{
		...
		$sql = "select * from sal_ka_busine where id=".$index." ";
	...
	protected function save(&$connection)
	{
		...
		switch ($this->scenario) {
			case 'delete':
				$sql = "delete from sal_ka_busine where id = :id";
				break;
			case 'new':
				$sql = "insert into sal_ka_busine(
						pro_name, z_index, z_display, city, lcu) values (
						:pro_name, :z_index, :z_display, :city, :lcu)";
				break;
			case 'edit':
				$sql = "update sal_ka_busine set 
					pro_name = :pro_name, 
					z_index = :z_index,
					z_display = :z_display,
					luu = :luu
    ...
    public static function getBusineNameForId($id){
        $row = Yii::app()->db->createCommand()->select("pro_name,id")->from("sal_ka_busine")
