...
class MarketAreaList extends CListPageModel
{
    public $employee_id;
	...
	public function retrieveDataByPage($pageNum=1)
	{
        ...
        $whereSql = " and (a.status_type in (5,6,8,10) or (a.status_type in (1,2) and a.z_index=3))";
        if($this->status_type!==""){//0:未分配 1：系统退回 2：手动退回 5：已分配 6：跟进中 8：无意向 10：已完成
            $this->status_type = is_numeric($this->status_type)?intval($this->status_type):0;
            switch ($this->status_type){
                case 0://未分配
                    $whereSql.=" and a.status_type=5 and a.allot_type=2";
                    break;
                case 1://退回
                    $whereSql.=" and a.status_type in (1,2) and a.z_index=3";
                    break;
                case 5://已分配
		...
		$sql1 = "select a.id,a.number_no,a.z_index,a.person_phone,a.company_name,a.allot_type,a.allot_city,a.allot_employee,
				...
				where a.allot_city='{$city}' {$whereSql}";
		...
		if (count($records) > 0) {
			foreach ($records as $k=>$record) {
                ...
                $this->attr[] = array(
                    ...
                    'color'=>MarketFun::getTrColorForArea($record),
                    'allot_type'=>$record["allot_type"],
                    'z_index'=>$record["z_index"],
                    'status_type'=>$record["status_type"],
                    'status_str'=>MarketFun::getStatusStrForArea($record),
                    'detail'=>$infoRows?$infoRows:array(),
                );
			}
		}
		$session = Yii::app()->session;
		$session['marketArea_c01'] = $this->getCriteria();
		return true;
	}
    ...
    public function countNotify(){
        $city=Yii::app()->user->city();
        //未分配
        $expr_sql = "(a.status_type=5 and a.allot_type=2)";
        //手动退回及系统退回
        $expr_sql.= " or (a.status_type in (1,2) and a.z_index=3)";
        $row = Yii::app()->db->createCommand()
            ->select("count(a.id)")
            ->from("sal_market a")
            ->where("a.allot_city='{$city}' and ({$expr_sql})")->queryScalar();
        return $row;
    }
}
