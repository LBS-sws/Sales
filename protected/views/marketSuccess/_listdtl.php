...
$idX = $this->record['id'];
?>
<tr class='clickable-row <?php echo $this->record['color']; ?>' data-href='<?php echo $this->getLink('MT03', 'marketSuccess/edit', 'marketSuccess/view', array('index'=>$this->record['id']));?>'>
    <td class="che">
        <?php if (empty($this->record["ready_bool"])): ?>
            <input value="<?php echo $this->record['id']; ?>"  type="checkbox">
        <?php endif ?>
    </td>
    <td><?php echo $this->drawEditButton('MT03', 'marketSuccess/edit', 'marketSuccess/view', array('index'=>$this->record['id'])); ?></td>
