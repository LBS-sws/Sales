...
$idX = $this->record['id'];
?>
<tr class='clickable-row <?php echo $this->record['color']; ?>' data-href='<?php echo $this->getLink('MT03', 'marketSales/edit', 'marketSales/view', array('index'=>$this->record['id']));?>'>

    <td><?php echo $this->drawEditButton('MT03', 'marketSales/edit', 'marketSales/view', array('index'=>$this->record['id'])); ?></td>
...
if (count($this->record['detail'])>0) {
    foreach ($this->record['detail'] as $row) {
        ...
        $html.= "<td colspan='3' class='text-right'><strong>".Yii::t("market","info date")."：</strong>".General::toDate($row["info_date"])."</td>";
