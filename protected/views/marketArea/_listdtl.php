...
$idX = $this->record['id'];
?>
<tr class='clickable-row <?php echo $this->record['color']; ?>' data-href='<?php echo $this->getLink('MT02', 'marketArea/edit', 'marketArea/view', array('index'=>$this->record['id']));?>'>
    <td class="che">
        <?php if (MarketFun::isAssignForArea($this->record)): ?>
            <input value="<?php echo $this->record['id']; ?>"  type="checkbox">
        <?php endif ?>
    </td>
    <td><?php echo $this->drawEditButton('MT02', 'marketArea/edit', 'marketArea/view', array('index'=>$this->record['id'])); ?></td>
