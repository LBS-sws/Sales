...
function renderContractPagination(pageNum, noOfPages, totalRow, search){
    ...
    if(totalRow > 0){
        ...
        // 上一页
        if(pageNum > 1){
            html += '<a href="javascript:void(0);" onclick="loadClientContract(' + (pageNum-1) + ', \'' + search + '\')"><上一页</a> ';
        }
        ...
        for(var i = 1; i <= noOfPages; i++){
            if(i == pageNum){
                ...
                html += '<a href="javascript:void(0);" onclick="loadClientContract(' + i + ', \'' + search + '\')">' + i + '</a> ';
            }
        }
        
        // 下一页
        if(pageNum < noOfPages){
            html += '<a href="javascript:void(0);" onclick="loadClientContract(' + (pageNum+1) + ', \'' + search + '\')">下一页></a> ';
        }
        
        // 页码跳转
        html += '跳转到 <input type="number" id="contractPageJump" min="1" max="' + noOfPages + '" style="width:50px;" /> 页 ';
        html += '<button onclick="gotoContractPage(' + noOfPages + ', \'' + search + '\')">跳转</button>';
        html += '</div>';
    }
