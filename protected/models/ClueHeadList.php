...
class ClueHeadList extends CListPageModel
{
    public $flow_odds="";
    ...
    public function searchColumns() {
        ...
        if (self::isReadAll()) {
            $search['rec_employee_id'] = "concat(f.code,' (',f.name,')')";
        }
        return $search;
    }
	...
	public function retrieveDataByPage($pageNum=1)
	{
		...
		$sql1 = "select a.*,sra.pro_name as clue_source_name,b.name as city_name,concat(f.name,' (',f.code,')') as employee_name,
				...
				where a.del_num=0 and a.table_type=1 and a.rec_type=1 ";
        ...
        $this->toDayNum = Yii::app()->db->createCommand()->select("count(a.id)")->from("sal_clue a")
            ->where(" a.del_num=0 and a.table_type=1 and a.rec_type=1 {$clause} and date_format(a.last_date,'%Y/%m/%d')='{$nowDate}'")
            ->queryScalar();
        switch ($this->flow_odds){
            ...
            case 2://下属负责的
                ...
                break;
            case 3://今日待跟进
                $clause.=" and date_format(a.last_date,'%Y/%m/%d')='{$nowDate}' ";
                break;
            case 4://今日已跟进
                $clause.=" and date_format(a.end_date,'%Y/%m/%d')='{$nowDate}' ";
                break;
            case 5://从未跟进的
                $clause.=" and a.end_date is null ";
                break;
        }
		...
		$session['criteria_ClueHeadList'] = $this->getCriteria();
		return true;
	}
