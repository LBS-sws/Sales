...
class SealForm extends CFormModel
{
	/* User Fields */
	public $id;
	public $name;
        ...
        if($scenario=="new"){
            ...
            if($scenario=="delete"){
                $clueFlowRow = Yii::app()->db->createCommand()->select("*")
                    ->from("sal_contract")
                    ->where("seal_type_id=:id",array(":id"=>$this->id))->queryRow();
                if($clueFlowRow){
	...
	public function retrieveData($index)
	{
		...
		$sql = "select * from sal_seal where id=".$index." ";
	...
	protected function save(&$connection)
	{
		...
		switch ($this->scenario) {
			case 'delete':
				$sql = "delete from sal_seal where id = :id";
				break;
			case 'new':
				$sql = "insert into sal_seal(
						name,mh_code, z_display, lcu, luu) values (
						:name,:mh_code, :z_display, :lcu, :luu)";
				break;
			case 'edit':
				$sql = "update sal_seal set 
					name = :name, 
					mh_code = :mh_code,
					z_display = :z_display,
					luu = :luu
