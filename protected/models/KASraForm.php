...
class KASraForm extends CFormModel
{
	/* User Fields */
	public $id;
	public $pro_name;
	...
	public function retrieveData($index)
	{
		...
		$sql = "select * from sal_ka_sra where id=".$index." ";
	...
	protected function save(&$connection)
	{
		...
		switch ($this->scenario) {
			case 'delete':
				$sql = "delete from sal_ka_sra where id = :id";
				break;
			case 'new':
				$sql = "insert into sal_ka_sra(
						pro_name, z_index, z_display, city, lcu) values (
						:pro_name, :z_index, :z_display, :city, :lcu)";
				break;
			case 'edit':
				$sql = "update sal_ka_sra set 
					pro_name = :pro_name, 
					z_index = :z_index,
					z_display = :z_display,
					luu = :luu
    ...
    public static function getSourceListForId($id=""){
        $list = array(""=>"");
        $rows = Yii::app()->db->createCommand()->select("pro_name,id")->from("sal_ka_sra")
    ...
    public static function getSourceNameForId($id){
        $row = Yii::app()->db->createCommand()->select("pro_name,id")->from("sal_ka_sra")
