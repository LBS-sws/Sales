...
class ImportClueForm extends ImportClientForm
{
    protected $pinyin;
    ...
    protected function saveOneData($data){
        $saveKey=array(
            'clue_type','service_type','cust_name','full_name','clue_code','abbr_code','entry_date','yewudalei','group_bool','rec_employee_id',
            'cust_vip','cust_class','cust_class_group','city','address','clue_source','district','street','cust_person','cust_tel','cust_email','cust_person_role','cust_address','area','clue_remark',
        );
        $saveList=array();
        ...
        Yii::app()->db->createCommand()->insert("sal_clue_history",array(
            ...
            "table_type"=>1,
            "history_type"=>1,
            "history_html"=>"<span>线索导入，导入id：{$this->id}</span>",
            "lcu"=>$this->username,
        ));
